        ...
        <Row :gutter="10">
            <Col span="24">
                <Card>
                    ...
                    <Row>
	                    <i-col span="12">
						<Button type="dashed"  @click="getalltest"  icon="md-bug" >测试数据</Button>
					        ...
					        <Upload action="http://localhost:8001/corpora-server/words/impexcel" :before-upload="handleImpexcelBeforeUpload" :on-success="handleImpexcelSuccess"
...
export default {
    ...
    methods: {
		...
		getalltest(){
        	let self = this;
        	self.$axios.post('http://localhost:8001/corpora-server/address111/getpagedata', {}, {
				headers: {
        ...
        handleSearch (topage) {
			...
			self.$axios.get('http://localhost:8001/corpora-server/words/pagedata?page='+current+'&size='+pagesize+'&sort='+sort+'&keywords='+keywords, {}, {}
				).then(function(response) {
					console.log("返回的数据1111111111111111111111111111111111111111111");
				console.log(response)
        ...
        handleEdit (type){
        	if(type=='add'){
				const route = {name: 'words-edit',query: {id: '',refresh:true}}
	            ...
	            if(selection.length==1){
		            const route = {name: 'words-edit',query: {id: selection[0].id,refresh:true}}
					this.addTag({route: route,type: 'push'})
					this.$router.push(route)
	            }
        	}
        ...
        handleExpexcel(){
        	...
        	let keywords = this.table.keywords;
        	window.location.href='http://localhost:8001/corpora-server/words/expexcel?sort='+sort+'&keywords='+keywords;
        },
        	...
        	if(selection.length>0){
        		this.$Modal.confirm({
        			...
        			onOk: function(){
			        	...
			        	self.$axios.delete('http://localhost:8001/corpora-server/words/delete?urlids='+urlids, {}, {
					    	headers: {
					        	"Content-Type":"application/json;charset=utf-8"
					       	},
					    }).then(function(response) {
    ...
    watch: {
        '$route' (to) {
            ...
            if(to.name=='words-manage' && to.query && to.query.refresh){
