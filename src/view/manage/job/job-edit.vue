    ...
    export default {
        ...
        methods: {
        	handleInit (){
	        	...
	        	if(id && id.length>0){
		        	self.$axios.get('http://localhost:8001/corpora-server/job/singledata?id='+id, {}, {
				    	headers: {
            ...
            handleSubmit (name) {
                ...
                this.$refs[name].validate((valid) => {
                    if (valid) {
						...
						self.$axios.post('http://localhost:8001/corpora-server/job/save', param, {}
						).then(function(response) {
	    ...
	    watch: {
	        '$route' (to) {
	            if(to.name=='job-edit' && to.query && to.query.refresh){
