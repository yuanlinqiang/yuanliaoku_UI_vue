                ...
                <Card>
                    ...
                    <Row>
	                    <i-col span="12">
					        ...
					        <Upload action="http://localhost:8001/corpora-server/person/impexcel" :before-upload="handleImpexcelBeforeUpload" :on-success="handleImpexcelSuccess"
...
export default {
    ...
    methods: {
        ...
        handleSearch (topage) {
			...
			self.$axios.get('http://localhost:8001/corpora-server/person/pagedata?page='+current+'&size='+pagesize+'&sort='+sort+'&keywords='+keywords, {}, {}
			...
			// axios.get('/person/pagedata?page='+current+'&size='+pagesize+'&sort='+sort+'&keywords='+keywords, {}, {
		    // 	headers: {
        ...
        handleEdit (type){
        	if(type=='add'){
				const route = {name: 'person-edit',query: {id: '',refresh:true}}
	            ...
	            if(selection.length==1){
		            const route = {name: 'person-edit',query: {id: selection[0].id,refresh:true}}
					this.addTag({route: route,type: 'push'})
					this.$router.push(route)
	            }
        	}
        ...
        handleExpexcel(){
        	...
        	let keywords = this.table.keywords;
        	window.location.href='http://localhost:8001/corpora-server/person/expexcel?sort='+sort+'&keywords='+keywords;
        },
        		...
        		this.$Modal.confirm({
        			...
        			onOk: function(){
			        	...
			        	for(let i=0;i<selection.length;i++){
			        		personids += flag + selection[i].id;
			        		flag = ',';
			        	}
			        	self.$axios.delete('http://localhost:8001/corpora-server/person/delete?personids='+personids, {}, {
					    	headers: {
					        	"Content-Type":"application/json;charset=utf-8"
					       	},
					    }).then(function(response) {
    ...
    watch: {
        '$route' (to) {
            ...
            if(to.name=='person-manage' && to.query && to.query.refresh){
