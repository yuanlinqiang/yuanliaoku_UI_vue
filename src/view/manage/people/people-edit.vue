    ...
    export default {
        ...
        data () {
            return {
                ...
                ruleValidate: {
					...
					area: [
						{ required: false, message: '网站指向地区不能为空', trigger: 'blur'  }
					],
					datasource: [
        ...
        methods: {
        	handleInit (){
	        	...
	        	if(id && id.length>0){
		        	self.$axios.get('http://localhost:8001/corpora-server/people/singledata?id='+id, {}, {
				    	headers: {
            ...
            handleSubmit (name) {
                ...
                this.$refs[name].validate((valid) => {
                    if (valid) {
						...
						self.$axios.post('http://localhost:8001/corpora-server/people/save', param, {}
						).then(function(response) {
	    ...
	    watch: {
	        '$route' (to) {
	            if(to.name=='people-edit' && to.query && to.query.refresh){
