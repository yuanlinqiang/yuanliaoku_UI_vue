                ...
                <Card>
                    ...
                    <Row>
	                    <i-col span="12">
					        ...
					        <Upload action="http://localhost:8001/corpora-server/people/impexcel" :before-upload="handleImpexcelBeforeUpload" :on-success="handleImpexcelSuccess"
...
export default {
    ...
    methods: {
		...
		handleDeleteAll (){
			...
			this.$Modal.confirm({
				loading:true,
				title:'数据删除操作确认',
				content:'<p>确认删除所有的人名？</p>',
				onOk: function(){
					let modal = this;
					let selection = self.table.selection;
					self.$axios.delete('http://localhost:8001/corpora-server/people/deleteAll', {}, {
						headers: {
        ...
        handleSearch (topage) {
			...
			self.$axios.get('http://localhost:8001/corpora-server/people/pagedata?page='+current+'&size='+pagesize+'&sort='+sort+'&keywords='+keywords, {}, {}
				).then(function(response) {
				console.log(response)
        ...
        handleEdit (type){
        	if(type=='add'){
				const route = {name: 'people-edit',query: {id: '',refresh:true}}
	            ...
	            if(selection.length==1){
		            const route = {name: 'people-edit',query: {id: selection[0].id,refresh:true}}
					this.addTag({route: route,type: 'push'})
					this.$router.push(route)
	            }
        	}
        ...
        handleExpexcel(){
        	...
        	let keywords = this.table.keywords;
        	window.location.href='http://localhost:8001/corpora-server/people/expexcel?sort='+sort+'&keywords='+keywords;
        },
        	...
        	if(selection.length>0){
        		this.$Modal.confirm({
        			...
        			onOk: function(){
			        	...
			        	self.$axios.delete('http://localhost:8001/corpora-server/people/delete?urlids='+urlids, {}, {
					    	headers: {
					        	"Content-Type":"application/json;charset=utf-8"
					       	},
					    }).then(function(response) {
    ...
    watch: {
        '$route' (to) {
            ...
            if(to.name=='people-manage' && to.query && to.query.refresh){
