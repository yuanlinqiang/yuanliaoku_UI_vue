    ...
    <div>
        <Row :gutter="10">
            <Col span="24">
                <Card>
                	<p slot="title">
                        <Icon type="ios-alert"></Icon> 数据爬取
                    </p>
                    <Row>
	                    <i-col span="12">
						<Button type="dashed"  @click="handleGetData"  icon="md-bug" >获取网址文本</Button>
					        ...
					        <Upload action="http://localhost:8001/corpora-server/allLink/impexcel" :before-upload="handleImpexcelBeforeUpload" :on-success="handleImpexcelSuccess"
...
export default {
    ...
    methods: {
		...
		handleGetData(){   //获取相关网站的文本信息
        	let self = this;
        	let selection = self.table.selection;
			//if(selection.length==0) this.$Message.warning('');
			if(selection.length>0){
			this.$Modal.confirm({
        			loading:true,
					title:'文本获取操作确认',
        			content:'<p>确认获取选中的网址文本数据？</p>',
        			onOk: function(){
        				let modal = this;
			        	let selection = self.table.selection;
			        	let urlids = '',flag = '';
			        	...
			        	self.$axios.post('http://localhost:8001/corpora-server/crawling/getDocText?urlids='+urlids, {}, {
					    	headers: {
					        	"Content-Type":"application/json;charset=utf-8"
					       	},
					    }).then(function(response) {
					    	modal.remove();
					    	self.$Message.warning('数据爬取成功!');
					        self.handleSearch();
				...
				this.$Modal.confirm({
        			loading:true,
					title:'获取文本操作确认',
        			content:'<p>确认爬取所有的网站文本数据？</p>',
        			onOk: function(){
						let modal = this;
			        	self.$axios.post('http://localhost:8001/corpora-server/crawling/getDocText', {}, {
					    	headers: {
					        	"Content-Type":"application/json;charset=utf-8"
					       	},
					    }).then(function(response) {
     	...
     	handleDeleteAll (){
			...
			this.$Modal.confirm({
				loading:true,
				title:'数据删除操作确认',
				content:'<p>确认删除所有的网站数据？</p>',
				onOk: function(){
					let modal = this;
					let selection = self.table.selection;
					self.$axios.delete('http://localhost:8001/corpora-server/allLink/deleteAll', {}, {
						headers: {
							"Content-Type":"application/json;charset=utf-8"
						},
					}).then(function(response) {
        ...
        handleSearch (topage) {
			...
			self.$axios.get('http://localhost:8001/corpora-server/allLink/pagedata?page='+current+'&size='+pagesize+'&sort='+sort+'&keywords='+keywords, {}, {}
				).then(function(response) {
				console.log(response)
        ...
        handleEdit (type){
        	if(type=='add'){
				const route = {name: 'alllink-edit',query: {id: '',refresh:true}}
	            ...
	            if(selection.length==1){
		            const route = {name: 'alllink-edit',query: {id: selection[0].id,refresh:true}}
					this.addTag({route: route,type: 'push'})
					this.$router.push(route)
	            }
        	}
        ...
        handleExpexcel(){
        	...
        	let keywords = this.table.keywords;
        	window.location.href='http://localhost:8001/corpora-server/allLink/expexcel?sort='+sort+'&keywords='+keywords;
        },
        	...
        	if(selection.length>0){
        		this.$Modal.confirm({
        			...
        			onOk: function(){
			        	...
			        	self.$axios.delete('http://localhost:8001/corpora-server/allLink/delete?urlids='+urlids, {}, {
					    	headers: {
					        	"Content-Type":"application/json;charset=utf-8"
					       	},
					    }).then(function(response) {
    ...
    watch: {
        '$route' (to) {
            ...
            if(to.name=='alllink-manage' && to.query && to.query.refresh){
