    ...
    export default {
        ...
        methods: {
        	handleInit (){
	        	...
	        	if(id && id.length>0){
		        	self.$axios.get('http://localhost:8001/corpora-server/crawling/singledata?id='+id, {}, {
				    	headers: {
            ...
            handleSubmit (name) {
                ...
                this.$refs[name].validate((valid) => {
                    if (valid) {
						...
						self.$axios.post('http://localhost:8001/corpora-server/crawling/save', param, {}
						).then(function(response) {
	    ...
	    watch: {
	        '$route' (to) {
	            if(to.name=='crawling-edit' && to.query && to.query.refresh){
