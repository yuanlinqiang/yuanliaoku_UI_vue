    ...
    <div>
        <Row :gutter="10">
            <Col span="24">
                <Card>
                	<p slot="title">
                        <Icon type="ios-alert"></Icon> 数据管理
                    </p>
                    <Row>
	                    <i-col span="12">
						<Button type="dashed"  @click="handleGetData"  icon="md-bug" >获取数据</Button>
					        ...
					        <Upload action="http://localhost:8001/corpora-server/crawling/impexcel" :before-upload="handleImpexcelBeforeUpload" :on-success="handleImpexcelSuccess"
...
export default {
    ...
    methods: {
		...
		handleDeleteAll (){
			...
			this.$Modal.confirm({
				...
				onOk: function(){
					...
					self.$axios.delete('http://localhost:8001/corpora-server/crawling/deleteAll', {}, {
						headers: {
			...
			if(selection.length>0){
        		this.$Modal.confirm({
        			...
        			onOk: function(){
			        	...
			        	self.$axios.post('http://localhost:8001/corpora-server/crawling/getAllUrlByLineName?urlids='+urlids, {}, {
					    	headers: {
					        	"Content-Type":"application/json;charset=utf-8"
					       	},
					    }).then(function(response) {
				...
				this.$Modal.confirm({
        			loading:true,
					title:'数据爬取操作确认',
        			content:'<p>确认爬取所有的网站数据？</p>',
        			onOk: function(){
						let modal = this;
			        	self.$axios.post('http://localhost:8001/corpora-server/crawling/getAllUrlByLineName', {}, {
					    	headers: {
					        	"Content-Type":"application/json;charset=utf-8"
					       	},
					    }).then(function(response) {
        ...
        handleSearch (topage) {
			...
			self.$axios.get('http://localhost:8001/corpora-server/crawling/pagedata?page='+current+'&size='+pagesize+'&sort='+sort+'&keywords='+keywords, {}, {}
				).then(function(response) {
				console.log(response)
        ...
        handleEdit (type){
        	if(type=='add'){
				const route = {name: 'crawling-edit',query: {id: '',refresh:true}}
	            ...
	            if(selection.length==1){
		            const route = {name: 'crawling-edit',query: {id: selection[0].id,refresh:true}}
					this.addTag({route: route,type: 'push'})
					this.$router.push(route)
	            }
        	}
        ...
        handleExpexcel(){
        	...
        	let keywords = this.table.keywords;
        	window.location.href='http://localhost:8001/corpora-server/crawling/expexcel?sort='+sort+'&keywords='+keywords;
        },
        	...
        	if(selection.length>0){
        		this.$Modal.confirm({
        			...
        			onOk: function(){
			        	...
			        	self.$axios.delete('http://localhost:8001/corpora-server/crawling/delete?urlids='+urlids, {}, {
					    	headers: {
					        	"Content-Type":"application/json;charset=utf-8"
					       	},
					    }).then(function(response) {
    ...
    watch: {
        '$route' (to) {
            ...
            if(to.name=='crawling-manage' && to.query && to.query.refresh){
